...
router.put('/items/:id', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    // 4. 只有 "available" 状态的物品才能被修改
    if (item.status !== 'available') {
        return res.status(400).json({ message: `物品处于 "${item.status}" 状态，无法修改` });
    }
    ...
    if (req.file) {
      ...
      if (item.imageUrl) {
        ...
        } catch (e) { console.warn("删除旧图片失败:", e); }
      }
      item.imageUrl = `${BASE_URL}/uploads/${req.file.filename}`;
    }
    ...
    if (item.metadataUrl) {
        ...
        } catch (e) { console.warn("删除旧元数据文件失败:", e); }
    }
    const metadataFileName = `${Date.now()}.json`;
    const metadataPath = path.join(metadataDir, metadataFileName);
    const newMetadataUrl = `${BASE_URL}/metadata/${metadataFileName}`;
    ...
    // 异步重新生成并保存 embedding
    generateAndSaveEmbedding(updatedItem);

    res.status(200).json({ 
        message: '物品信息更新成功，并已同步到区块链', 
        data: updatedItem,
        txHash: txHash
    });
...
router.delete('/items/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: '归档物品失败', error: err.message });
  }
});
